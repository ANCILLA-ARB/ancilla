...
| Relay-server hosting | 🟡 **config ready, not deployed.** [`relay-server/Dockerfile`](relay-server/Dockerfile), a dedicated `build:relay-server` script, and Fly.io/Railway configs now exist — the compiled binary was proven live through a full commit → HTTP → reveal → on-chain-confirm cycle (see [`relay-server/DEPLOYMENT.md`](relay-server/DEPLOYMENT.md)). Along the way, found and fixed a real bug: `ethers`/`dotenv` were misclassified as dev-only dependencies, which would have broken a production-only install. What's still missing is the actual hosting decision (which provider, whose account, what budget) — deliberately left to the project owner rather than a place for an agent to create accounts or spend money unilaterally. Non-blocking regardless: the protocol never depends on the relay — an agent can always call `commitIntent`/`revealIntent` directly. |
